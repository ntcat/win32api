...
//
// 	var handle api.PDH_HQUERY
// 	var counterHandle api.PDH_HCOUNTER
// 	ret := api.PdhOpenQuery(0, 0, &handle)
//	ret = api.PdhAddEnglishCounter(handle, "\\Processor(_Total)\\% Idle Time", 0, &counterHandle)
//	var derp api.PDH_FMT_COUNTERVALUE_DOUBLE
//
//	ret = api.PdhCollectQueryData(handle)
//	fmt.Printf("Collect return code is %x\n", ret) // return code will be PDH_CSTATUS_INVALID_DATA
//	ret = api.PdhGetFormattedCounterValueDouble(counterHandle, 0, &derp)
//
//	ret = api.PdhCollectQueryData(handle)
...
//	var bufSize uint32
//	var bufCount uint32
//	var size uint32 = uint32(unsafe.Sizeof(api.PDH_FMT_COUNTERVALUE_ITEM_DOUBLE{}))
//	var emptyBuf [1]api.PDH_FMT_COUNTERVALUE_ITEM_DOUBLE // need at least 1 addressable null ptr.
//
//	for {
//		// collect
//		ret := api.PdhCollectQueryData(queryHandle)
//		if ret == api.ERROR_SUCCESS {
//			ret = api.PdhGetFormattedCounterArrayDouble(counterHandle, &bufSize, &bufCount, &emptyBuf[0]) // uses null ptr here according to MSDN.
//			if ret == api.PDH_MORE_DATA {
//				filledBuf := make([]api.PDH_FMT_COUNTERVALUE_ITEM_DOUBLE, bufCount*size)
//				ret = api.PdhGetFormattedCounterArrayDouble(counterHandle, &bufSize, &bufCount, &filledBuf[0])
//				for i := 0; i < int(bufCount); i++ {
//					c := filledBuf[i]
//					var s string = api.UTF16PtrToString(c.SzName)
