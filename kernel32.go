...
func GetLocaleInfo(Locale LCID, LCType LCTYPE, lpLCData LPTSTR, cchData int32) int32 {
...
func GetLogicalDriveStrings(nBufferLength DWORD, lpBuffer LPTSTR) DWORD {
...
// 	return HGLOBAL(ret)
// }
func GlobalAlloc(uFlags DWORD, dwBytes DWORD) HGLOBAL {
	ret, _, _ := syscall.Syscall(globalAlloc.Addr(), 2,
		uintptr(uFlags),
		uintptr(dwBytes),
		0)
...
func OpenProcess(dwDesiredAccess uint32, bInheritHandle uint32, dwProcessId uint32) int32 {
...
func VirtualAllocEx(hProcess, lpAddress, dwSize uint32,
	flAllocationType *uint32, flProtect uint32) uint32 {
	ret, _, _ := syscall.Syscall6(virtualAllocEx.Addr(), 5,
		uintptr(hProcess),
		uintptr(lpAddress),
		uintptr(dwSize),
		uintptr(unsafe.Pointer(flAllocationType)),
		uintptr(flProtect),
		0)
...
func VirtualFreeEx(hProcess, lpAddress, dwSize uint32, dwFreeType int) uint32 {
	ret, _, _ := syscall.Syscall6(virtualAllocEx.Addr(), 4,
		uintptr(hProcess),
		uintptr(lpAddress),
		uintptr(dwSize),
		uintptr(dwFreeType),
...
func WriteProcessMemory(hProcess HANDLE, lpBaseAddress LPVOID,
	lpBuffer LPVOID, nSize DWORD, lpNumberOfBytesWritten LPDWORD) bool {
